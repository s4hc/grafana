    ...
    var result = {
      ...
      init: function(dashboard) {
        ...
        if(dashboard.services && dashboard.services.filter) {
          ...
          this.oldTemplateParameters = this.templateParameters || [];
          this.templateParameters = dashboard.services.filter.list || [];

          // S4HC AJ: Preserve selected filters if they exist
          this.templateParameters = _.map(this.templateParameters, function(filter){
            var match = _.findWhere(self.oldTemplateParameters, {name : filter.name, query:filter.query});
